...
mod tests {
    ...
    #[test]
    fn test_git_pull_remote_without_upstream_returns_error() {
        let dir = local_repo_with_note();
        git_command()
            .args(["remote", "add", "origin", "/tmp/example.git"])
            .current_dir(dir.path())
            .output()
            .unwrap();

        let result = git_pull(path_text(dir.path())).unwrap();

        assert_eq!(result.status, "error");
        assert!(result.message.contains("No upstream branch configured"));
    }
