...
/// Get the current branch label and how many commits it is ahead/behind its
/// configured upstream. Repositories with remotes but no tracking branch are
/// reported explicitly instead of being collapsed into a clean remote state.
...
mod tests {
    ...
    #[test]
    fn git_remote_status_with_remote_but_no_upstream() {
        let dir = local_repo_with_note();
        super::super::command::run_git(
            dir.path(),
            &["remote", "add", "origin", "/tmp/example.git"],
        )
        .unwrap();
