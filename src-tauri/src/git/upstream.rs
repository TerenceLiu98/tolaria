...
pub(crate) fn missing_upstream_message(vault: &Path) -> Result<String, String> {
    ...
    if branch == DETACHED_HEAD_BRANCH {
        return Ok(
            "This vault is in detached HEAD. Check out a branch and configure its upstream before syncing in Sapientia."
                .to_string(),
        );
    }
    Ok(format!(
        "No upstream branch configured for '{branch}'. Set a tracking branch with external Git tooling, then sync again in Sapientia."
    ))
}
