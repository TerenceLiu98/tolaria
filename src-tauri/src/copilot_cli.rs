...
fn format_copilot_error(stderr_output: &str, status: &str) -> String {
    if is_auth_or_setup_error(stderr_output) {
        return "GitHub Copilot CLI is not ready. Run `copilot login` in your terminal, then run `copilot` from this vault folder and trust it before retrying in Sapientia.".into();
    }
...
mod tests {
    ...
    fn copilot_mcp_config_uses_tolaria_stdio_server() {
        ...
        let config = copilot_mcp_config_json(CopilotMcpConfigInput {
            request: &request,
            mcp_server_path: "/opt/sapientia/mcp-server/index.js",
        ...
        assert_eq!(server["args"][0], "/opt/sapientia/mcp-server/index.js");
        assert_eq!(server["tools"][0], "*");
        assert_eq!(server["env"]["VAULT_PATH"], "/tmp/vault");
        assert!(server["env"]["VAULT_PATHS"]
            .as_str()
            .unwrap()
            .contains("/team-vault"));
    }
    ...
    #[test]
    fn format_copilot_error_reports_auth_setup() {
        assert!(format_copilot_error("please login first", "exit status: 1")
            .contains("GitHub Copilot CLI is not ready"));
    }
}
