...
export function useResolvedVaultExpressionTemplate(source: string): ResolvedVaultExpressionTemplate {
  ...
  const contentsByPath = useVaultExpressionDependencyContents(compiled, activeContext)

  return useMemo(() => renderVaultExpressionTemplate({
    compiled,
    context: {
      contentsByPath,
      currentContent: activeContext.currentContent,
      entries: activeContext.entries,
      locale: activeContext.locale,
      sourceEntry: activeContext.sourceEntry,
    },
  }), [activeContext, compiled, contentsByPath])
}
