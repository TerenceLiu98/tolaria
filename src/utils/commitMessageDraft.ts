...
const COMMIT_MESSAGE_SYSTEM_PROMPT = [
  'Draft one concise Git commit message from Sapientia vault changes.',
...
export function generateDeterministicCommitMessage(files: ModifiedFile[]): string {
  ...
  return basicMessage.replace(/\bnotes$/, `${fileKind(files)}${scope}`)
}
...
function diffSummaryPrompt(files: ModifiedFile[], fallback: string, excerpts: DiffExcerpt[]): string {
  ...
  return [
    'Draft a Git commit message for these Sapientia vault changes.',
...
export async function generateCommitMessageDraft({
  ...
  try {
    ...
    // Generation should never block committing.
  }
