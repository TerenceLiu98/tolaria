...
function sanitizeParsedHtml(documentObject: Document): string {
  documentObject.body.querySelectorAll('*').forEach((element) => {
    for (const attribute of REMOTE_LOADING_ATTRIBUTES) element.removeAttribute(attribute)
    ...
    if (element instanceof HTMLStyleElement) element.textContent = stripCssRemoteLoads(element.textContent ?? '')
