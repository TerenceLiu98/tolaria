...
`paper.md` is the canonical entity note. `source.pdf` is immutable app-managed input, while `blocks.jsonl` and `annotations.jsonl` are durable sidecar conventions for parsing and annotation phases. A missing sidecar means the corresponding phase has not produced that artifact yet.
...
Successful parsing also rewrites the parser-owned Markdown body of `paper.md` as a human-readable projection of the parsed paper. Each projected section is preceded by a hidden anchor comment such as `<!-- tolaria:block id="b0001" page="1" kind="paragraph" hash="sha256:..." -->`. The anchor is the bridge between Tolaria's normal note-reading model and block-addressable research workflows. Shared display and snippet paths strip those comments, so the anchors stay durable on disk without becoming visible reading text. Raw/source mode intentionally preserves them. User comments, highlights, and long-form notes are not written into this projected body.
...
`src/paper/PaperReaderShell.tsx` is the reader projection for Paper entries. It parses `paper.md` metadata and anchored Markdown sections, reads SourceBlocks through the `blocks.jsonl` command boundary, embeds the resolved `source.pdf` through the existing file preview path, and renders the result as PDF readiness, parsed paper-structure status, and a continuous Paper note view. The central Markdown reading surface prefers the anchored `paper.md` projection; `blocks.jsonl` supports citation validation, annotation grouping, block-focus lookup, and repair diagnostics. Missing/empty parsed structure stays recoverable, while malformed sidecars still expose structured diagnostic errors. When parsed structure is missing, the Reader exposes the parser action and reloads SourceBlocks after `parse_paper` succeeds. With the MinerU provider selected, the action is labeled "Parse with MinerU"; failed parse metadata renders a retry state with provider error detail without hiding existing loaded blocks.

The reader normalizes SourceBlock kinds for display as `title`, `heading`, `paragraph`, `figure`, `table`, `equation`, and `caption`. `src/paper/paperReaderBlocks.ts` owns sidecar-health and block display helpers so those conventions stay separate from the durable SourceBlock sidecar contract. Paper Markdown generation renders extracted figures as normal Markdown images, cleans leaked internal math sentinels into display math, lightly normalizes LaTeX spacing, and converts simple tab-delimited table text into Markdown tables. Each rendered block has a stable DOM anchor of `paper-block-<block_id>`. Block selection is local UI state; copying citations uses the canonical formatter, annotations attach to selected blocks through the comment gutter/thread, and pending citation navigation focuses the matching block when available. If a selected block has a page number, the shell records and displays a PDF page focus request in PDF mode; direct PDF page seeking, block overlays, and PDF-coordinate annotation rendering remain separate future surfaces.

`src/components/NoteSurface.tsx` is the shared note rendering surface used by ordinary notes and Paper Markdown mode. Ordinary notes use it as the existing editable `SingleEditorView` path. Paper passes `editable={false}` plus comment options so parsed `paper.md` reads like a normal Tolaria note while remaining parser-owned source content.

`src/comments/commentProvider.ts` is the generic Note Editor comment seam. It models anchors and comments without knowing how a backing store persists them. `src/components/comments/CommentUI.tsx` provides reusable comment gutter, thread, and composer components. Phase 4F wires only Paper to this seam through `src/paper/paperCommentProvider.ts`, which adapts block-level PaperAnnotation records from `annotations.jsonl` into generic comment threads. Normal notes do not use the seam yet because Tolaria still needs a durable generic note-block anchor convention before comments can safely survive edits and Markdown round-trips.

`annotations.jsonl` is the line-oriented PaperAnnotation sidecar for block-local Paper Reader annotations. Each line includes `id`, `paper_id`, `kind`, `created_at`, and either a `block_id` or future `page` plus `bbox` target. Phase 3A supports `highlight`, `underline`, `question`, `comment`, and `bookmark` kinds with semantic colors `questioning`, `important`, `original`, `pending`, and `conclusion`. Tolaria validates and rewrites the sidecar through active-vault-bound Tauri commands (`read_paper_annotations`, `save_paper_annotation`, `delete_paper_annotation`, `reset_paper_annotations`) while leaving `source.pdf` immutable. The Paper Reader creates block-level annotations from the rendered block gutter, edits annotation kind/color/note text inline in a block thread, shows counts beside SourceBlocks, and treats missing/empty/malformed annotation sidecars as recoverable UI states.

Comments, annotations, ResearchNotes, and future memory are distinct abstractions. A generic comment is UI around an anchor plus provider-backed mutations. A Paper comment is currently represented as a block-level `PaperAnnotation` with `kind: comment` stored in `annotations.jsonl`; highlights, questions, underlines, and bookmarks are also PaperAnnotations and can share the same block thread. Long-form synthesis belongs in ordinary Markdown `ResearchNote` entries and can contain durable `@block[...]` citations back to the paper. Future memory artifacts are compiled/synthesized research context and must preserve provenance instead of becoming the storage location for comments or annotations.

Paper no longer owns a default paper-local note convention or Reader action. Users create ordinary `ResearchNote` entries through Tolaria's normal note workflows when they need long-form notes about a paper. Those notes may cite parsed Paper blocks with `@block[paper_id#block_id]`, but Paper Reader does not create, preview, or append to a special note.
