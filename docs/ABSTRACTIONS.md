...
`paper.md` is the canonical entity note. `source.pdf` is immutable app-managed input, while `blocks.jsonl`, `annotations.jsonl`, and `metadata.json` are durable sidecar conventions for parsing, comments, and metadata resolution. A missing sidecar means the corresponding phase has not produced that artifact yet.
