...
`CanvasGraphicsCommandBatch` is the body-free rendering contract for connectors and transient previews. The controller builds it from `CanvasSceneStore.queryEdges()` rather than handing the full scene to React. It resolves both connector endpoints through NodeSpec-owned anchors and carries the chosen anchor ids with screen-independent Canvas points; an invalid empty custom anchor set falls back to the deterministic top/right/bottom/left geometry. The current `CanvasGraphicsLayer` keeps SVG as a backend, batches visible visual strokes into paths, and emits a fixed 12px transparent hit region per visible connector; tools and persistence do not know which graphics backend executes the commands. `CanvasDocumentLayer` retains only visible/active DOM nodes, and `CanvasOverlayLayer` keeps selection controls at stable pixel sizes regardless of zoom.
