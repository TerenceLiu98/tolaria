...
`src/paper/PaperReaderShell.tsx` is the minimal reader projection for Paper entries. It parses `paper.md` metadata, reads SourceBlocks through the `blocks.jsonl` command boundary, embeds the resolved `source.pdf` through the existing file preview path, and renders the result as PDF readiness, parsed paper-structure status, and a collapsible Paper outline. Missing/empty parsed structure stays recoverable, while malformed sidecars still expose structured diagnostic errors. When parsed structure is missing, the Reader exposes the parser action and reloads SourceBlocks after `parse_paper` succeeds. With the MinerU provider selected, the action is labeled "Parse with MinerU"; failed parse metadata renders a retry state with provider error detail without hiding existing loaded blocks. Block selection is local UI state; copying citations uses the canonical formatter, and pending citation navigation focuses the matching outline row when available. PDF block overlays and PDF-coordinate annotation rendering are separate future surfaces, not part of the current reader shell.
