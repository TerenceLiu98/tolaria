...
`src/paper/PaperReaderShell.tsx` is the reader projection for Paper entries. It parses `paper.md` metadata, reads SourceBlocks through the `blocks.jsonl` command boundary, embeds the resolved `source.pdf` through the existing file preview path, and renders the result as PDF readiness, parsed paper-structure status, a collapsible Paper outline, and a readable block view. Missing/empty parsed structure stays recoverable, while malformed sidecars still expose structured diagnostic errors. When parsed structure is missing, the Reader exposes the parser action and reloads SourceBlocks after `parse_paper` succeeds. With the MinerU provider selected, the action is labeled "Parse with MinerU"; failed parse metadata renders a retry state with provider error detail without hiding existing loaded blocks.

The reader normalizes SourceBlock kinds for display as `title`, `heading`, `paragraph`, `figure`, `table`, `equation`, and `caption`. `src/paper/paperReaderBlocks.ts` owns the UI-only outline, search, display-kind, page-label, and sidecar-health helpers so those conventions stay separate from the durable SourceBlock sidecar contract. The outline is derived from titles, headings, and first blocks on each page; in-paper search covers text, captions, sections, kinds, and ids. Block selection is local UI state; copying citations uses the canonical formatter, annotations attach to selected blocks, and pending citation navigation focuses the matching block when available. If a selected block has a page number, the shell records and displays a PDF page focus request; direct PDF page seeking, block overlays, and PDF-coordinate annotation rendering remain separate future surfaces.

`annotations.jsonl` is the line-oriented PaperAnnotation sidecar for user marginalia. Each line includes `id`, `paper_id`, `kind`, `created_at`, and either a `block_id` or future `page` plus `bbox` target. Phase 3A supports `highlight`, `underline`, `question`, `comment`, and `bookmark` kinds with semantic colors `questioning`, `important`, `original`, `pending`, and `conclusion`. Tolaria validates and rewrites the sidecar through active-vault-bound Tauri commands (`read_paper_annotations`, `save_paper_annotation`, `delete_paper_annotation`, `reset_paper_annotations`) while leaving `source.pdf` immutable. The Paper Reader currently creates block-level annotations for all supported kinds/colors, edits annotation kind/color/note text inline, shows counts on SourceBlock rows, and treats missing/empty/malformed annotation sidecars as recoverable UI states.

Paper-local ResearchNotes are normal Markdown notes under `papers/<paper-slug>/notes/`, not sidecars. The default marginalia note is `papers/<paper-slug>/notes/marginalia.md`; it carries `type: ResearchNote` and a `paper:` wikilink to `[[papers/<paper-slug>/paper]]`. The Paper Reader default action creates that note if missing or opens it if present. When a SourceBlock is selected, the reader can append the canonical `@block[paper_id#block_id]` citation to the marginalia note through the same note-content command boundary. The Reader's Marginalia mode can read that note for an in-shell preview, report missing/ready/error state without creating it, and refresh the preview after an append while leaving full rich editing in the normal editor surface. Explicit future "new paper note" creation should allocate `marginalia-2.md`, `marginalia-3.md`, and so on instead of changing the default open-existing behavior.
