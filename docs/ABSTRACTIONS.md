...
- `CanvasHistoryManager` records one reversible transaction for a completed drag, resize, connect/reconnect, group/reparent, graph paste, delete, or auto-layout operation. BlockNote undo is a separate history domain. Controller group commands preserve explicit `parentId` hierarchy, reject reparenting a group into its own descendants, promote surviving children when a containing group is removed, and move active-group focus to the parent on exit. Canvas copy expands selected groups to their descendants, delegates node serialization to each NodeSpec, includes only edges whose endpoints are in the copied subgraph, and remaps node ids, edge ids, and parent references on paste. Markdown bodies and Paper sidecars never enter this graph clipboard.
