...
Project notes mount `ProjectWorkspaceSurface` and enter Canvas directly; the former `Note | Canvas` switch no longer defines two independent product surfaces. `PROJECT_OVERVIEW_NODE_ID` identifies the required `project_overview` node, a normal `note` reference to the Project's own Markdown path. TypeScript, Rust, mocks, and MCP normalization create this node for new Canvases and repair it in legacy Canvases while preserving its geometry. The root node cannot be deleted. `ProjectCanvasSurface` shows bounded cards for referenced Notes, Papers, Paper blocks, images, text cards, tasks, and groups; draws lightweight relationship edges; and persists pan/zoom plus node geometry through the Project Canvas commands. Missing Canvas files are created automatically. `ProjectDocumentPreview` loads full Markdown only for the selected Note/Paper through `noteContentCache`, strips frontmatter, and renders a bounded static preview; low zoom and unselected nodes do not trigger document loads. Double-clicking one document node replaces that preview with the single `CanvasEditorPortal`. The portal owns one editor configured by `useSapientiaBlockNoteEditor`, mounts the shared `NoteSurface` into the active node, preserves frontmatter during Markdown serialization, and saves the referenced path through the app save boundary. Switching, closing, or deleting the active node flushes pending document work first. Focus Mode moves the same portal between the node and a Canvas-local full workspace frame; it does not remount a second editor or open another tab, and exiting restores the node editor. Inactive nodes remain static previews, Canvas JSON never stores editor state, and Inspector `Open` remains the explicit standalone-editor exit. The bottom toolbar adds existing objects and embedded cards, while the inspector edits selected objects. Duplicate refs focus the existing node.
