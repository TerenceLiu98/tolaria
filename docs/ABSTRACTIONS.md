...
`annotations.jsonl` is the line-oriented PaperAnnotation sidecar for block-local Paper comments. Each line includes `id`, `paper_id`, `kind`, `created_at`, and either a `block_id` or future `page` plus `bbox` target. New Paper comments are stored with `kind: comment`. Tolaria validates and rewrites the sidecar through active-vault-bound Tauri commands (`read_paper_annotations`, `save_paper_annotation`, `delete_paper_annotation`, `reset_paper_annotations`) while leaving `source.pdf` immutable. The Paper Reader creates block-level comments from the rendered block gutter, edits comment text inline in a block thread, shows counts beside SourceBlocks, and treats missing/empty/malformed annotation sidecars as recoverable UI states. Legacy kind/color values from earlier Paper Reader iterations are preserved if present, but they are no longer exposed as product controls.

Comments, annotations, ordinary notes, and future memory are distinct abstractions. A generic comment is UI around an anchor plus provider-backed mutations. A Paper comment is currently represented as a block-level `PaperAnnotation` with `kind: comment` stored in `annotations.jsonl`. Long-form synthesis belongs in ordinary Markdown `Note` entries that link to Paper through wikilinks/backlinks and can contain durable `@block[...]` citations back to the paper. Future memory artifacts are compiled/synthesized research context and must preserve provenance instead of becoming the storage location for comments.
