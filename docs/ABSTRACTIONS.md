...
- `CanvasSceneStore` normalizes node/edge records, ordering, group membership references, bounds, coarse node/connector spatial indexes, node adjacency, and incident-edge lookup. `serialize()` sorts records deterministically for Git-friendly JSON and stores no document body. Pointer-frequency geometry uses incremental cell patches and reindexes only incident connectors; selected, editing, gesture-target, connected, and selected-edge records can be retained without a full-scene comparison.
...
`CanvasGraphicsCommandBatch` is the body-free rendering contract for connectors and transient previews. The controller builds it from `CanvasSceneStore.queryEdges()` rather than handing the full scene to React. The current `CanvasGraphicsLayer` keeps SVG as a backend, batches visible visual strokes into paths, and emits a fixed 12px transparent hit region per visible connector; tools, persistence, and NodeSpecs do not know which graphics backend executes the commands. `CanvasDocumentLayer` retains only visible/active DOM nodes, and `CanvasOverlayLayer` keeps selection controls at stable pixel sizes regardless of zoom.
...
`CanvasOverlayCoordinator` is the single source for viewport clipping, screen-space rects, stable z-order, snap guides, contextual toolbar placement, focus owner, outside-pointer dismissal, and Escape dismissal priority. Consumers render its snapshot; they do not measure or reposition overlays by querying transformed DOM nodes. The acceptance performance contract is explicit: low zoom caps mounted DOM nodes at 180, document previews at 80, and images at 24, while retained active nodes bypass those limits. Low zoom uses 24px bounded overscan and 128px spatial cells; the deterministic 5,000-node browser fixture requires fewer than 500 spatial candidates, samples frame gaps below 50ms, and checks heap growth when Chromium exposes `performance.memory`.
