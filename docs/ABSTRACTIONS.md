...
- `CanvasLayerManager`, `CanvasOverlayCoordinator`, and `CanvasNodeSpecRegistry` keep graphics, document DOM, screen-space UI, and node behavior extensible without engine-level renderer conditionals. Each NodeSpec selects a renderer adapter and supplies preview, editor geometry, Inspector fields/actions, contextual toolbar actions, drop/clipboard behavior, add-input construction, Navigator section metadata, auto-layout policy, navigation, resize capability, stable connection-anchor geometry, and stale-state rendering. `ProjectCanvasNodeRendererRegistry` binds those adapters to concrete React renderers for Overview/document, Paper block, image, text, task, and group content. `ProjectCanvasNodeCard` renders only the shared card chrome and delegates node content and optional Paper metadata subtitle through the spec/renderer boundary; it does not branch on node type or renderer kind. Low zoom applies bounded DOM/document/image budgets, and overlay handles and contextual toolbar controls remain fixed-size in screen space.
...
The shipped NodeSpec registry makes behavior executable for Project Overview, Note, Paper, Paper block, image, text, task, and group nodes. `ProjectCanvasController.createNode`, `createNodeFromInput`, `addDropValue`, `addDropPayload`, `autoLayout`, and `createPeekNode` consume the registry for geometry, editor geometry, IDs, input/drop resolution, auto-layout columns/minimums, stale-safe rendering metadata, and placement. Missing explicit positions use the true Canvas point at the viewport center; temporary UI geometry uses the distinct centered-top-left helper. The Canvas root only chooses application-level entry candidates and dispatches commands, while Navigator and Inspector read their node-specific labels/icons/actions from the same registry.
