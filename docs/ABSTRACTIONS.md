...
Project notes mount `ProjectWorkspaceSurface` and enter Canvas directly; the former `Note | Canvas` switch no longer defines two independent product surfaces. `PROJECT_OVERVIEW_NODE_ID` identifies the required `project_overview` node, a normal `note` reference to the Project's own Markdown path. TypeScript, Rust, mocks, and MCP normalization create this node for new Canvases and repair it in legacy Canvases while preserving its geometry. The root node cannot be deleted. `ProjectCanvasSurface` remains a compact spatial renderer in the first ADR-0174 phase: it shows bounded cards for referenced Notes, Papers, Paper blocks, images, text cards, tasks, and groups; draws lightweight relationship edges; persists pan/zoom plus node geometry through the Project Canvas commands; and routes node clicks through existing Sapientia navigation. Missing Canvas files are created automatically. The bottom floating toolbar can add existing Notes/Papers, short text/task/group cards, image refs, and `@block[...]` evidence nodes, while the inspector edits selected objects. Duplicate refs focus the existing node. Long-form bodies remain in Markdown, and the planned `CanvasEditorPortal` will mount one shared `NoteSurface` for only the active document node rather than storing editor state in Canvas JSON or mounting one editor per node.
