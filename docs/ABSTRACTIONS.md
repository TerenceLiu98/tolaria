...
`CanvasOverlayCoordinator` is the single source for viewport clipping, screen-space rects, stable z-order, snap guides, contextual toolbar placement, focus owner, and dismissal. Consumers render its snapshot; they do not measure or reposition overlays by querying transformed DOM nodes. The acceptance performance contract is explicit: low zoom caps mounted DOM nodes at 180, document previews at 80, and images at 24, while retained active nodes bypass those limits. The deterministic 5,000-node regression fixture requires fewer than 500 spatial candidates for its narrow query.
