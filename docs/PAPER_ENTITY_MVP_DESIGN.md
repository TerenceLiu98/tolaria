...
## Editable Paper Markdown

Paper-as-Note is the primary reading model. `paper.md` is not just metadata plus a PDF pointer; after a successful parse its Markdown body is the human-readable Paper Note. Parsing creates or refreshes the initial anchored Markdown, but `paper.md` is editable through the same Note surface as ordinary notes. An explicit reparse may replace the generated Paper content after user confirmation, while normal note editing remains user-owned. The stored Markdown keeps hidden `tolaria:block` comments for block identity, but shared display paths strip those comments before rendering, snippets, or previews so users read the paper rather than the machine anchors. Raw/source mode still shows the anchors because that mode is the filesystem truth.
...
Phase 2C originally added the first Paper Reader surface without changing the underlying files-first model. At that point, opening a `type: Paper` entity rendered `src/paper/PaperReaderShell.tsx` inside the existing editor content area instead of mounting the generic rich-note editor. Raw mode still showed the original `paper.md` Markdown unchanged.

That historical reader shell displayed:
