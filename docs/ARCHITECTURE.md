...
- **Project Canvas engine acceptance boundary**: `ProjectCanvasController` is the only durable Canvas mutation boundary. `CanvasSceneStore` uses incremental geometry patches, 128px node cells, 512px connector cells, and stable snapshots; `CanvasViewport` keeps a 24px bounded low-zoom overscan separate from hit-test bounds; and `CanvasLayerManager` caps low-zoom mounts at 180 DOM nodes, 80 document previews, and 24 images while retaining selected, editing, connected, gesture-target, and overlay-owned nodes. `CanvasNodeSpecRegistry` owns renderer behavior adapters and node-specific toolbar/inspector/drop/clipboard/resize contracts. `CanvasOverlayCoordinator` owns clipped screen-space selection chrome, shadcn contextual actions, snap guides, focus ownership, z-order, and outside/Escape dismissal. Browser acceptance loads deterministic 5,000-node scenes through the project Canvas command handlers and verifies fewer than 500 spatial candidates, stable DOM budgets, sub-50ms sampled frame gaps, and optional heap-growth bounds; the 1,000-node spatial fixture remains a fast unit gate. External native Tauri, CodeScene, and Codacy evidence is a release-environment prerequisite before ADR 0175 changes to fully accepted.
...
`CanvasOverlayCoordinator` clips screen-space selection chrome, handles, toolbars, and guides to the viewport and publishes deterministic z-order, focus owner, and dismissal state. The overlay layer is bounded by the viewport and keeps control sizes independent of camera zoom. At low zoom, the layer manager enforces 180 mounted DOM nodes, 80 document previews, and 24 images; active selection, editing, gesture, connected, and overlay-owned nodes are retained. Deterministic 5,000-node fixtures verify spatial candidate counts stay below 500 for a narrow query and that active off-screen nodes survive culling.
