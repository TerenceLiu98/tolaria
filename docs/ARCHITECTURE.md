...
ADR 0176 starts capability alignment with executable node behavior ownership. `CanvasNodeSpecRegistry` declares each existing node's renderer adapter, reference editing mode, preview, geometry, Inspector fields, toolbar actions, clipboard behavior, and drop resolver. `ProjectCanvasNodeRendererRegistry` maps those adapters to concrete React components. The common `ProjectCanvasNodeCard` owns selection and temporary-node chrome but does not choose document, citation, image, text, task, or group content with node-type branches; the Inspector likewise follows the NodeSpec reference mode instead of renderer checks. This preserves the one-editor portal and external Markdown/Paper ownership while making renderers independently replaceable.
...
Acceptance hardening keeps focus ownership explicit: the portal reports document focus to the controller, while Canvas focus restores the Canvas history domain without clearing either stack. `CanvasViewport` is the only client-to-screen/canvas conversion boundary, and controller helpers also own centered placement, graphics bounds, editor geometry, and Peek placement. The surface does not construct durable or Peek nodes and does not resolve dropped content; those operations use NodeSpec-backed controller commands.
