...
The ADR 0176 graphics path uses `CanvasSceneStore.queryEdges()` to obtain only connector candidates intersecting the render bounds, plus explicitly retained selected edges. Pointer-frequency geometry patches reindex only incident connectors. `ProjectCanvasController.queryVisibleGraphics()` converts those records and the active connection preview into renderer-independent `CanvasGraphicsCommandBatch` values. The current SVG backend combines normal and selected visuals into batched paths and keeps separate 12px transparent hit regions; tools and persistence do not depend on SVG, leaving anchors, routing, and a later Canvas2D backend behind the same command boundary.
