...
- **Paper entities**: Research PDFs are imported as Paper bundles under `papers/<paper-slug>/`. Phase 1 creates `source.pdf` and `paper.md`, with the Paper note carrying `type: Paper`, `paper_id`, `source_pdf`, and future sidecar filenames. The root `paper.md` Type document makes Papers appear through the existing type section/list system; no custom Paper list store or database is introduced. Block citations use durable Markdown syntax (`@block[paper_id#block_id]`) parsed in `src/paper/blockCitations.ts`; rendered citation clicks record pending block focus and open the matching Paper entity. Paper entries render through `src/paper/PaperReaderShell.tsx`, which shows metadata, source PDF preview/status, SourceBlock sidecar health, a block outline, citation copy actions, requested block focus, and block-level annotation counts/actions. Annotation persistence uses active-vault Tauri commands over `annotations.jsonl` (`read_paper_annotations`, `save_paper_annotation`, `delete_paper_annotation`, `reset_paper_annotations`) and keeps `source.pdf` immutable while leaving PDF-coordinate overlays for later phases. Paper-local ResearchNotes are ordinary Markdown notes under `papers/<paper-slug>/notes/`; `src/paper/marginalia.ts` owns the default `marginalia.md` template, Paper wikilink convention, unique fallback naming, and selected-block citation append behavior while routing opens back through the existing note navigation path.
