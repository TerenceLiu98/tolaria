...
- **Project Canvas**: Project Canvas is a local-first organization layer for `type: Project` notes, introduced by ADR-0173. The canvas file lives at `projects/<project-id>/project.canvas.json` for bundle-style Projects or as an adjacent `<project>.canvas.json` file beside a Project Markdown note. `src-tauri/src/project_canvas.rs` and `src/projectCanvas.ts` define the shared schema, node kinds (`note`, `paper`, `paper_block`, `text`, `task`, `group`), edge kinds (`related`, `supports`, `contradicts`, `depends_on`, `needs_reading`), stable JSON ordering, and reference-resolution diagnostics. The canvas owns position, size, viewport, and relationship edges; it does not own Note/Paper bodies or derived snippets. Tauri commands (`create_project_canvas`, `read_project_canvas`, `save_project_canvas`, `resolve_project_canvas_refs`, and the helper `project_canvas_paths`) stay inside the active-vault boundary, reject non-Project notes, validate graph integrity on save, and degrade stale Note/Paper/block references into structured diagnostics instead of failing the whole canvas. The UI mounts a `ProjectEditorSurface` for Project notes, letting users switch between the normal editable Project Note and `ProjectCanvasSurface`. The canvas surface can create/open the canvas file, render typed node cards and relationship edges, pan/zoom, drag/resize nodes, drag from a node connector to create an edge, persist viewport and geometry through the command boundary, and navigate referenced objects through the existing note/Paper routing. A bottom floating toolbar owns canvas-local add/search actions, fit-to-view, and auto layout; the right-side canvas inspector edits selected node titles/text and selected edge kind/notes, and can delete selected nodes or edges. Duplicate Note/Paper refs focus the existing node instead of creating another copy. It intentionally does not embed full editors, Paper readers, PDFs, or AI panels inside nodes.
