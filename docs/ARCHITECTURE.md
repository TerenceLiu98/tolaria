...
- **Paper entities**: Research PDFs are imported as Paper bundles under `papers/<paper-slug>/`. Phase 1 creates `source.pdf` and `paper.md`, with the Paper note carrying `type: Paper`, `paper_id`, `source_pdf`, and future sidecar filenames. The root `paper.md` Type document makes Papers appear through the existing type section/list system; no custom Paper list store or database is introduced. Block citations use durable Markdown syntax (`@block[paper_id#block_id]`) parsed in `src/paper/blockCitations.ts`; rendered citation clicks record pending block focus and open the matching Paper entity. Paper entries render through `src/paper/PaperReaderShell.tsx`, which shows metadata, PDF readiness/preview, parsed paper-structure state, a collapsible Paper outline, a readable SourceBlock view, in-paper search, citation copy actions, requested block/page focus, and block-level annotation counts/actions. Parser provider selection is installation-local settings (`none`, `dev-fixture`, `mineru`), and the active-vault-bound `parse_paper` command writes normalized `blocks.jsonl`, updates Paper parse metadata, and leaves `source.pdf` immutable. The MinerU provider uses an installation-local API token directly or resolves it from an environment variable name, uploads `source.pdf` through MinerU's remote batch flow, polls for output, extracts `content_list.json` from the returned artifact, normalizes blocks, and preserves an existing valid `blocks.jsonl` when parsing fails. The shell has Read and Marginalia modes: Read keeps the PDF preview beside the Paper outline and rendered block view, while Marginalia keeps the outline and shows a preview/actions pane for the paper-local marginalia note without mounting a second full editor. If a selected SourceBlock has a page number, the shell records and displays PDF focus intent; direct PDF page seeking and coordinate overlays are intentionally deferred. Annotation persistence uses active-vault Tauri commands over `annotations.jsonl` (`read_paper_annotations`, `save_paper_annotation`, `delete_paper_annotation`, `reset_paper_annotations`) and keeps `source.pdf` immutable while leaving PDF-coordinate overlays for later phases. Paper-local ResearchNotes are ordinary Markdown notes under `papers/<paper-slug>/notes/`; `src/paper/marginalia.ts` owns the default `marginalia.md` template, Paper wikilink convention, unique fallback naming, read-only preview lookup, and selected-block citation append behavior while routing opens back through the existing note navigation path.
