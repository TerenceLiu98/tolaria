...
| Localization | App-owned runtime + manually maintained JSON catalogs (`src/lib/i18n.ts`, `src/lib/locales/*.json`) | English fallback + `pnpm l10n:validate` |
...
- **Paper entities**: Research PDFs are imported as Paper bundles under `papers/<paper-slug>/`. Phase 1 creates `source.pdf` and `paper.md`, with the Paper note carrying `type: Paper`, `paper_id`, `source_pdf`, and sidecar filenames. The root `paper.md` Type document makes Papers appear through the existing type section/list system; no custom Paper list store or database is introduced. Parser provider selection is installation-local settings (`none`, `dev-fixture`, `mineru`), and the active-vault-bound `parse_paper` command writes normalized `blocks.jsonl`, writes or refreshes editable `paper.md` as readable Markdown with hidden `tolaria:block` anchors, updates Paper parse metadata, and leaves `source.pdf` immutable. Default parse calls refuse to overwrite `parse_status: parsed` Papers; the Reader's confirmed reparse action passes `force: true` so replacement is explicit. `blocks.jsonl` is the machine index for lookup, citation validation, comment grouping, and repair; editable anchored `paper.md` is the main human reading artifact, and shared display paths strip the hidden anchor comments before rendering, indexing snippets, or showing previews. `metadata.json` stores canonical Paper metadata, provider provenance, candidate matches, confidence, and resolver errors; `paper.md` frontmatter mirrors only user-facing fields such as title, authors, year, venue, DOI, arXiv id, metadata status, and confidence. Metadata commands (`read_paper_metadata`, `extract_paper_metadata`, `refresh_paper_metadata`, `apply_paper_metadata_candidate`, `save_paper_metadata`) stay inside the active-vault boundary and never mutate `source.pdf`; extraction uses local PDF/Markdown signals to find DOI, arXiv id, and title, then uses OpenAlex as the primary remote resolver with exact DOI lookup before title search. Manual saves are recorded as reviewed metadata and clear pending candidates. The derived Paper catalog (`src/paper/catalog.ts` and `src-tauri/src/paper/catalog.rs`) rebuilds `PaperCatalogEntry[]` from Paper frontmatter plus optional sidecar metadata and never becomes a source-of-truth database. Catalog commands (`list_paper_catalog`, `search_paper_catalog`, `find_paper_duplicates`, `refresh_paper_catalog`, `mark_paper_duplicate_decision`) stay inside the active-vault boundary; only user duplicate-dismissal review state is persisted, under `papers/catalog-decisions.json`. The Papers type list uses that derived catalog to add metadata search and compact filters for venue, venue type, parse status, metadata status, and duplicate candidates while opening Paper notes through the normal note path. The MinerU provider uses an installation-local API token directly or resolves it from an environment variable name, uploads `source.pdf` through MinerU's remote batch flow, polls for output, extracts `content_list.json` plus images from the returned ZIP artifact when present, writes extracted images under `papers/<paper-slug>/assets/`, normalizes figure/table blocks with asset paths, appends fallback figure blocks for unreferenced image assets, writes standard Markdown image syntax into `paper.md`, and preserves existing parsed artifacts when parsing fails. Block citations use durable Markdown syntax (`@block[paper_id#block_id]`) parsed in `src/paper/blockCitations.ts`; rendered citation clicks record pending block focus and open the matching Paper entity. Paper entries render through a thin `src/paper/PaperReaderShell.tsx` wrapper that keeps normal readiness state out of the header, keeps explicit `Parse Paper` and `Paper metadata` actions in the header, asks before replacing existing parsed content, and keeps metadata review/edit/refresh inside one bounded metadata dialog. The Markdown Reading View mounts `paper.md` through the shared editable `src/components/NoteSurface.tsx` path with Paper comments enabled instead of duplicating ordinary note rendering logic. The comment surface is split into a generic seam (`src/comments/commentProvider.ts` plus `src/components/comments/CommentUI.tsx`) and a Paper adapter (`src/paper/paperCommentProvider.ts`) that maps `comments.jsonl` records to comment threads by block anchor; normal notes do not use this seam until a durable note-block anchor model exists. The seam stays a compact gutter and opens the selected thread as a local floating panel instead of reserving a permanent wide comment column. If a selected SourceBlock has a page number, the shell records and displays PDF focus intent in PDF mode; direct PDF page seeking and coordinate overlays are intentionally deferred. Comment persistence uses active-vault Tauri commands over `comments.jsonl` (`read_paper_comments`, `save_paper_comment`, `delete_paper_comment`, `reset_paper_comments`); the command names are compatibility API names, while the sidecar schema is comment-only and keeps `source.pdf` immutable while leaving PDF-coordinate overlays for later phases. Long-form user synthesis is handled by ordinary `Note` entries linked to Paper through wikilinks/backlinks and optional `@block[...]` citations; Paper no longer creates, previews, or appends to a special note.
- **Project Workspace**: ADR-0174 supersedes ADR-0173's separate Project Note and optional Canvas modes. Opening a `type: Project` entry now mounts `ProjectWorkspaceSurface` and enters Canvas directly. Every new or legacy Canvas is normalized with one reserved `project_overview` Note node that references the Project's own Markdown path; the node cannot be deleted, while its geometry remains user-controlled. The canvas file lives at `projects/<project-id>/project.canvas.json` for bundle-style Projects or as an adjacent `<project>.canvas.json` file beside a Project Markdown note. `src-tauri/src/project_canvas.rs`, `src/projectCanvas.ts`, and `mcp-server/project-canvas-model.js` define the shared schema, required Overview migration, node kinds (`note`, `paper`, `paper_block`, `image`, `text`, `task`, `group`), edge kinds (`related`, `supports`, `contradicts`, `depends_on`, `needs_reading`), stable JSON ordering, and reference-resolution diagnostics. The canvas owns position, size, viewport, short embedded card content, task completion state, and relationship edges; it does not own Note/Paper bodies or derived snippets. Missing Canvas files are created automatically when the Project workspace opens. Tauri commands (`create_project_canvas`, `read_project_canvas`, `save_project_canvas`, `resolve_project_canvas_refs`, and the helper `project_canvas_paths`) stay inside the active-vault boundary, reject non-Project notes, validate graph integrity on save, and degrade stale Note/Paper/block/image references into structured diagnostics instead of failing the whole canvas. `addNodeToProjectCanvas()` remains the shared application boundary for external add flows and deduplicates stable refs. `ProjectCanvasSurface` renders lightweight typed cards and relationship edges with pan/zoom, drag/resize, connector editing, dropped content, undo/redo, fit-to-view, auto layout, and a compact inspector. Its fixed left `ProjectCanvasNavigator` groups the same in-memory node list by type and focuses nodes without persisting a second membership index. `projectCanvasViewport.ts` measures the viewport and keeps only intersecting or overscan nodes mounted; selected and actively edited nodes are retained even when outside the measured bounds, while the full graph remains available to edges, navigation, persistence, and the Navigator. Low zoom uses an overview presentation with no body preview. Selecting one Note or Paper loads only that document through the bounded note-content cache and renders its Markdown body with `ProjectDocumentPreview`; unselected nodes keep snippets and do not load full content. Double-clicking a document activates one `CanvasEditorPortal`, enlarges undersized nodes to a usable editor frame, and mounts the shared `NoteSurface` into that node. `useSapientiaBlockNoteEditor` is the common BlockNote configuration for standalone and Canvas editors, so Markdown, math, wikilinks, images, paste behavior, formatting, and selected-text AI context stay aligned. Active Papers route that same portal editor through `PaperReaderShell`, reusing its Markdown/PDF source switch, metadata and parse commands, exact block citations, and `comments.jsonl` thread UI rather than duplicating Paper state in Project Canvas. The portal preserves frontmatter and submits arbitrary referenced paths through the existing app save boundary; target switches, Escape, and active-node deletion flush pending document saves. Focus Mode moves that same portal into a Canvas-local full workspace frame through `Cmd/Ctrl+Enter` or the Canvas toolbar and returns it to the node without changing the Markdown target or viewport. Editor and preview wikilinks resolve persistent Canvas members before standalone navigation. Resolved vault documents outside the Project open as one transient Peek node; Peek is excluded from Canvas JSON and undo history, while Pin adds the reference through normal Canvas persistence and deduplication. Block-citation navigation keeps the existing pending source-block focus request but routes the resolved Paper path into the active Project, so the Canvas focuses an existing Paper node or opens a Paper Peek before `PaperReaderShell` scrolls to the exact block. Only one Canvas editor exists at a time, while Inspector `Open` remains the explicit standalone navigation action. `ProjectCanvasAiContext` continues to bound selected and one-hop context with exact `@block` provenance.
- **Project Canvas engine acceptance boundary**: `ProjectCanvasController` is the only durable Canvas mutation boundary. `CanvasSceneStore` uses incremental geometry patches, 128px node cells, 512px connector cells, and stable snapshots; `CanvasViewport` keeps a 24px bounded low-zoom overscan separate from hit-test bounds; and `CanvasLayerManager` caps low-zoom mounts at 72 DOM nodes, 40 document previews, and 16 images while retaining selected, editing, connected, gesture-target, and overlay-owned nodes. The Navigator is independently virtualized and keeps at most 80 rows mounted, so accessibility and off-screen navigation do not restore a full-scene DOM list. `CanvasNodeSpecRegistry` owns renderer behavior adapters and node-specific toolbar/Inspector/drop/clipboard/resize contracts together with add-input construction, auto-layout policy, Navigator section order/labels/icons, and Inspector actions. `CanvasOverlayCoordinator` owns clipped screen-space selection chrome, shadcn contextual actions, snap guides, focus ownership, z-order, and outside/Escape dismissal. `ProjectCanvasPersistenceAdapter` serializes viewport and structural writes through one ordered queue: a camera write already in flight must finish before a newer structural transaction begins, `flush()` waits for the active file command, and the toolbar remains in a saving or explicit failed state until every queued command settles. Browser acceptance loads deterministic mixed 1,000- and 5,000-node scenes through the project Canvas command handlers, performs real Space-drag and wheel gestures, and fails when spatial candidates reach 500, Canvas DOM exceeds the retained 72-node budget, Navigator rows exceed 80, sampled frame gaps reach 50ms, interaction commits exceed 20, or Chromium heap growth exceeds the fixture-specific bound. The 2026-07-14 Chromium baseline mounted 63 Canvas nodes in both scenes, queried 241/481 candidates, mounted 21/28 Navigator rows, sampled 24.8/25ms pan gaps and 17/9.4ms zoom gaps, and grew the measured heap by approximately 2.9/5.95MB. External native Tauri, CodeScene, and Codacy evidence remains a release-environment prerequisite; these browser measurements do not mark the wider AFFiNE-alignment matrix accepted.
...
Paper-aware context stays compact. When the active note is a Paper, `src/utils/ai-context.ts` includes the Paper id, bibliographic frontmatter, sidecar names, parse/metadata status, and comment-sidecar location. Ordinary notes that wikilink to Papers get a small `relatedPapers` summary, and notes containing `@block[...]` syntax get resolved citation targets when the Paper is present in the current vault snapshot. `paperAiContextSummary()` derives the visible/debug surface for the existing AI panel: Paper tools available, Paper context included, active Paper metadata for hover preview, resolved citation count, related Paper count, selected Paper/block identifiers when available, and read-only mounted Paper Vault availability. The editor can expose selected text as `selectedContext`, but the AI panel includes it only after the user presses the selected-text context button, keeping incidental DOM selections out of prompts. Full Paper text and parsed blocks are intentionally left to explicit MCP Paper tools so model prompts do not silently absorb whole papers.

Paper-aware AI guidance is injected into the existing agent prompt rather than a new chat surface. Agents are told to use `search_papers` for discovery, `read_paper_metadata` before bibliographic claims, `search_paper_blocks` and `read_paper_blocks` for evidence, `get_block_citation` for exact citations, and to resolve `@block[...]` before discussing cited evidence. The guidance also tells agents to cite exact Paper blocks for Paper-grounded claims, say when evidence is insufficient, avoid dumping whole papers by default, and treat mounted Paper Vaults as read-only unless Tolaria exposes an explicit write path. Adding an AI response to a Project sends a transient draft intent rather than mutating Canvas immediately; the active Project renders a temporary Markdown preview, and explicit `Save as Note` creates the normal Markdown Note through `useNoteCreation` before persisting only its reference in Canvas JSON.
...
Each CLI agent authenticates itself outside Sapientia. Claude Code uses its existing CLI login or user-managed Anthropic/provider environment variables; Codex surfaces a friendly prompt to run `codex login` when needed; OpenCode surfaces a friendly prompt to run `opencode auth login` or configure a provider when needed; Pi surfaces a friendly prompt to run `pi /login` or configure a provider API key when needed. Sapientia does not store model-provider API keys in app settings; direct provider secrets stay in local app data or user-managed environment variables. App-managed Pi sessions copy that local Pi agent config into a per-run temporary directory before adding Sapientia MCP, so Sapientia does not overwrite global Pi files and does not drop a working standalone Pi setup.
...
Phase 5A adds read-only Paper tools to the same Node service instead of introducing a separate Paper agent server. `mcp-server/paper-tools.js` derives Paper catalog entries from `paper.md` frontmatter plus optional `metadata.json`, parses `blocks.jsonl` line by line, and returns compact, provenance-rich results. Registered tools are `search_papers`, `list_papers`, `read_paper_metadata`, `read_paper_outline`, `search_paper_blocks`, `read_paper_blocks`, `get_paper_citation`, and `get_block_citation`. Search tools return snippets; block reads require explicit ids or ranges and cap large reads with structured truncation metadata. Cross-vault reads follow the existing active mounted workspace set and include `vaultPath`/`vaultLabel`; ambiguous Paper ids require `vaultPath`. Paper tools do not write to vaults, so mounted Paper Vaults remain read-only through this boundary. Phase 5B tightens the registered tool descriptions so agents know when to discover Papers, when to read metadata, when to search/read blocks for evidence, what arguments to provide, what provenance appears in responses, and that truncation is intentional.
Project Canvas tools follow the same derived-file boundary. `mcp-server/project-canvas-storage.js` discovers every `type: Project` note and its canonical or adjacent Canvas, while `project-canvas-context.js` resolves only selected/one-hop context. Registered tools are `read_project_canvas`, `search_project_canvas`, `read_project_context`, and `add_node_to_project_canvas`. Reads span active mounted vaults and preserve Project/vault provenance; ambiguous identifiers require `vaultPath`. The add tool requires an explicit target when multiple vaults are active, creates a missing Canvas, deduplicates stable refs, and never writes into an implicitly selected mounted vault.
...
| `read_project_canvas` | `projectId, [vaultPath]` | Read one compact Project Canvas graph with provenance |
| `search_project_canvas` | `query, [projectId], [limit], [vaultPath]` | Search compact Canvas node metadata across active Projects |
| `read_project_context` | `projectId, [selectedNodeId], [vaultPath]` | Resolve selected/one-hop Project context with bounded Note and exact Paper-block evidence |
| `add_node_to_project_canvas` | `projectId, node, [vaultPath]` | Add or focus a Canvas node, creating the Canvas when missing; requires an unambiguous write target |
...
| `register_mcp(vault_path)` | Resolves an MCP runtime (Node.js 18+ preferred, Bun 1+ fallback), resolves the packaged or stable extracted `mcp-server/`, and writes Sapientia's vault-neutral entry to Claude Code, Antigravity CLI, Cursor, OpenCode, and generic MCP configs on user request |
| `mcp_config_snippet(vault_path)` | Builds the exact vault-neutral `mcpServers.tolaria` JSON users can copy into any compatible client without writing third-party config files; `tolaria` remains the compatibility server key while UI text says Sapientia MCP |
| `remove_mcp()` | Removes Sapientia's MCP entry from Claude Code, Antigravity CLI, Cursor, OpenCode, and generic MCP configs |
...
Sapientia's app chrome uses an app-owned localization runtime in `src/lib/i18n.ts`, backed by flat JSON catalogs in `src/lib/locales/` (see [ADR-0168](adr/0168-manual-json-catalog-localization.md)). `en.json` is the canonical source catalog, locale files are one file per locale, and English remains the fallback for any missing locale file or key. Catalogs are maintained manually and validated locally with `pnpm l10n:validate`; external localization services such as Lara are not part of the active localization workflow. The installation-local `ui_language` setting stores an explicit locale when the user chooses one; `null` means "follow the system language when Sapientia supports it, otherwise English." Legacy stored values such as `zh-Hans` are normalized to canonical locale codes like `zh-CN`.
...
`CanvasSceneStore` remains body-free: Markdown, Paper sidecars, comments, BlockNote documents, and rendered previews never enter Canvas JSON. Its node and connector spatial cells plus adjacency indexes answer viewport, hit-test, retention, and visible-edge queries without scanning the full scene during pointer updates; geometry patches update incident connector cells incrementally and controller snapshots reuse the stable scene view. `CanvasViewport` distinguishes rendering overscan from exact hit-testing bounds and batches camera changes through animation frames. The React surface uses `CanvasGraphicsLayer`, `CanvasDocumentLayer`, `CanvasOverlayLayer`, and `ProjectCanvasToolbar`; only the active `CanvasEditorPortal` mounts a live document editor. Structural commands persist promptly, viewport writes are debounced, low-zoom DOM/image budgets are enforced by `CanvasLayerManager`, and Canvas history remains separate from BlockNote document history.

Group hierarchy uses the existing optional Canvas-only `parentId`; it does not imply Markdown or editor-document containment. `ProjectCanvasController` owns group creation, enter/exit focus, reparenting, deletion promotion, graph copy/paste, alignment, distribution, and stacking. Group NodeSpecs expose an Enter action, double-click enters the group, and fixed-size screen-space navigation exits one hierarchy level at a time; Escape follows the same hierarchy after gesture, overlay, editor, and selection dismissal. Nodes created or dropped while a group owns focus inherit that group. A completed drag resolves the deepest eligible group from point-spatial candidates, reparents only selected hierarchy roots, rejects self/descendant cycles, and records geometry plus membership as one transaction. TypeScript, Rust, and MCP save/read validators reject missing parents, non-group parents, cyclic group ancestry, and invalid stacking values. Copying a group expands its descendant subtree, serializes each node through its NodeSpec, preserves internal connectors, and remaps node, edge, and parent ids in one paste transaction. A parent outside the copied subgraph is deliberately dropped, so pasted content remains a valid root subgraph without acquiring an accidental reference to the original group. The shadcn multi-selection toolbar dispatches one reversible transaction for each align, distribute, bring-to-front, or send-to-back action. Explicit optional integer `zIndex` values survive TypeScript, Rust, and MCP round trips; legacy nodes default to zero, and normalization uses `(zIndex, id)` for deterministic Git-friendly rendering order.

ADR 0176 starts capability alignment with executable node behavior ownership. `CanvasNodeSpecRegistry` declares each existing node's renderer adapter, reference editing mode, preview, geometry, Inspector fields/actions, toolbar actions, clipboard behavior, drop resolver, add-input constructor, Navigator metadata, and auto-layout policy. `ProjectCanvasNodeRendererRegistry` maps those adapters to concrete React components. The common `ProjectCanvasNodeCard` owns selection and temporary-node chrome but does not choose document, citation, image, text, task, group, or Paper metadata content with node-type branches; the Inspector and Navigator likewise consume the selected spec instead of maintaining parallel type tables. This preserves the one-editor portal and external Markdown/Paper ownership while making renderers independently replaceable.

The ADR 0176 graphics path uses `CanvasSceneStore.queryEdges()` to obtain only connector candidates intersecting the render bounds, plus explicitly retained selected edges. Pointer-frequency geometry patches reindex only incident connectors. `ProjectCanvasController.queryVisibleGraphics()` converts those records and the active connection preview into renderer-independent `CanvasGraphicsCommandBatch` values. Connector endpoints resolve through each NodeSpec's stable connection-anchor geometry; command records retain the selected anchor ids while persistence remains node-to-node and renderer-independent. Selecting a connector projects its two anchors into fixed-size screen-space overlay handles. Dragging either handle enters an explicit reconnect gesture, hides the old visual while previewing the fixed anchor to the pointer, rejects self/duplicate targets, commits one history transaction on a valid node, and restores the original edge on Escape or pointer cancellation. The optional edge routing field preserves legacy straight lines by default and adds orthogonal and curved paths across TypeScript, Rust, and MCP validation. Orthogonal obstacle lookup is spatially bounded to the connector corridor; route detours and conservative edge-index bounds share a 256-Canvas-unit cap so routed paths remain discoverable near viewport boundaries without full-scene work. Canvas-owned connector presentation adds an optional short label, solid/dashed stroke, `1 | 2 | 4` width, and independent none/arrow/circle/diamond/triangle endpoint markers. Legacy records default to an unlabeled solid 2px connector with no endpoint markers. The visible label is distinct from the relationship note and remains a reviewable scalar in Canvas JSON; it is deliberately not a Y.Text, Markdown body, or BlockNote document. The current SVG backend batches normal and selected visuals by routing/presentation signature, emits labels only for labeled visible connectors, and keeps separate 12px transparent hit paths; tools and persistence do not depend on SVG, leaving a later Canvas2D backend behind the same command boundary. The fixed Project Navigator remains independently virtualized over all scene membership: Arrow Up/Down and Home/End move keyboard focus by logical row without mutating selection, while activating a row selects it and asks the controller to reveal it. A pending row transfers focus from its Virtuoso mount ref, so keyboard navigation can reach a culled object without DOM lookup or timing retries; controller retention mounts the selected object while the camera catches up.

Acceptance hardening keeps focus ownership explicit: the portal reports document focus to the controller, while Canvas focus restores the Canvas history domain without clearing either stack. `CanvasViewport` is the only client-to-screen/canvas conversion boundary and exposes distinct viewport-center and centered-top-left helpers; controller helpers own durable-node placement, graphics bounds, editor geometry, and Peek placement. `createNodeFromInput()` delegates task defaults, group titles, text trimming, image references, and block citations to the selected NodeSpec. The surface no longer computes durable-node geometry or assembles embedded node fields; it dispatches the chosen node type and input through the controller.

`CanvasOverlayCoordinator` clips screen-space selection chrome, handles, toolbars, and guides to the viewport and publishes deterministic z-order, focus owner, and dismissal state. The overlay layer is bounded by the viewport and keeps control sizes independent of camera zoom. At low zoom, the layer manager enforces 72 mounted DOM nodes, 40 document previews, and 16 images; active selection, editing, gesture, connected, and overlay-owned nodes are retained. The Navigator uses a separate virtualized 80-row budget. Deterministic 1,000- and 5,000-node browser fixtures verify bounded spatial candidates, DOM mounts, Surface commits, pan/zoom frame gaps, and Chromium heap growth while retaining an off-screen selected node; engine fixtures separately verify edited, connected, dragged, and overlay-owned retention.
