...
- **Project Canvas**: Project Canvas is a local-first organization layer for `type: Project` notes, introduced by ADR-0173. The canvas file lives at `projects/<project-id>/project.canvas.json` for bundle-style Projects or as an adjacent `<project>.canvas.json` file beside a Project Markdown note. `src-tauri/src/project_canvas.rs` and `src/projectCanvas.ts` define the shared schema, node kinds (`note`, `paper`, `paper_block`, `image`, `text`, `task`, `group`), edge kinds (`related`, `supports`, `contradicts`, `depends_on`, `needs_reading`), stable JSON ordering, and reference-resolution diagnostics. The canvas owns position, size, viewport, short embedded card content, task completion state, and relationship edges; it does not own Note/Paper bodies or derived snippets. Tauri commands (`create_project_canvas`, `read_project_canvas`, `save_project_canvas`, `resolve_project_canvas_refs`, and the helper `project_canvas_paths`) stay inside the active-vault boundary, reject non-Project notes, validate graph integrity on save, and degrade stale Note/Paper/block/image references into structured diagnostics instead of failing the whole canvas. `addNodeToProjectCanvas()` is the shared application boundary for external add flows: it creates a missing target canvas, normalizes refs, adds at the viewport center, and focuses an existing node when the stable ref is already present. `ProjectCanvasAddProvider` supplies the shared Project picker to Note/Paper context menus, rendered `@block[...]` citations, and AI answers. A successful add emits a Project Canvas navigation intent so `ProjectEditorSurface` opens Canvas mode and `ProjectCanvasSurface` reloads and focuses the returned node. The canvas surface itself renders typed node cards and relationship edges, supports pan/zoom, drag/resize, connector-based edge creation, bounded image previews, dropped text/image/citation input, undo/redo, fit-to-view, auto layout, and a compact inspector. It intentionally does not embed full editors, Paper readers, PDFs, or AI panels inside nodes. Phase 5 derives a bounded `ProjectCanvasAiContext` for active Project notes: the selected node and one-hop neighbors are carried through a cross-window selection store, Paper nodes contribute metadata only, Note nodes contribute bounded snippets, and `paper_block` nodes resolve exact SourceBlock evidence with page and canonical `@block` provenance. The AI panel exposes compact Project/Paper/citation/stale counts and localized actions to summarize the Project, recommend the next Paper, or generate a cited outline through the existing prompt queue.
