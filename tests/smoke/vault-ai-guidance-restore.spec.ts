...
test('vault guidance restore command recovers missing managed guidance', async ({ page }) => {
  ...
  await expect(aiBadge).toHaveAttribute('title', /Sapientia guidance missing or broken/)

  await openCommandPalette(page)
  expect(await findCommand(page, 'Restore Sapientia AI Guidance')).toBe(true)
  await page.keyboard.press('Enter')

  await expect(page.getByText('Sapientia AI guidance restored')).toBeVisible()

  await aiBadge.click()
  await expect(page.getByTestId('status-ai-guidance-summary')).toHaveText('Sapientia guidance ready')
  await expect(page.getByTestId('status-ai-guidance-restore')).toHaveCount(0)
  await page.keyboard.press('Escape')

  await openCommandPalette(page)
  expect(await findCommand(page, 'Restore Sapientia AI Guidance')).toBe(false)
})
